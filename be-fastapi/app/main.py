...
async def _ensure_seed_user() -> None:
    if not settings.ENABLE_SEED_USER:
        return

    if not all(
        [
            settings.SEED_USER_USERNAME.strip(),
            settings.SEED_USER_FULL_NAME.strip(),
            settings.SEED_USER_EMAIL.strip(),
            settings.SEED_USER_PASSWORD.strip(),
        ]
    ):
        logger.warning("Seed user bootstrap skipped because credentials are incomplete")
        return
